...
const VOLE_GENIE_CHANCE = 0.05; // 5%
...
const QUESTIONS = [
    ...
    { q: "Quel est l'élément chimique dont le symbole est O ?", a: ["oxygène", "oxygene"] },
    { q: "Combien font 12 + 15 ?", a: ["27"] },
    { q: "Quelle couleur obtient-on en mélangeant rouge et bleu ?", a: ["violet"] },
...
module.exports = {
    ...
    checkActiveEvent: async (client, db) => {
        ...
        if (status && status.value === 'true') {
            ...
            if (now < endTime) {
                ...
                console.log(`[Events] L'Heure de Gloire restaurée. Fin dans ${(endTime - now) / 1000}s`);
    ...
    startGloryHour: async (client, db) => {
        ...
        if (channel) {
            ...
            embed.setImage('https://media.giphy.com/media/v1.Y2lkPTc5MGI3NjExbmhzcXN5MXB5eWx5eWx5eWx5eWx5eWx5eX/l0Ex6kAKp0YlFr1hc/giphy.gif'); // Generic hype gif or use generate_image if allowed/needed
    ...
    triggerVolDeGenie: async (message, db, targetUser, force = false) => {
        // 5% chance or forced
        if (!force && Math.random() > VOLE_GENIE_CHANCE) return { triggered: false };
        ...
        try {
            ...
            if (questionObj.a.includes(answer)) {
                await replyMsg.edit({ 
                    embeds: [createEmbed('✅ Accès Autorisé', 'Sécurité désactivée. Le vol procède avec un bonus !', COLORS.SUCCESS)] 
                });
                return { triggered: true, success: true };
            } else {
                await replyMsg.edit({ 
                    embeds: [createEmbed('❌ Accès Refusé', 'Alerte déclenchée ! La police arrive.', COLORS.ERROR)] 
                });
            ...
            await replyMsg.edit({ 
                embeds: [createEmbed('⏰ Temps écoulé', 'Le système de sécurité vous a bloqué.', COLORS.ERROR)] 
            });
            return { triggered: true, success: false };
        }
    }
