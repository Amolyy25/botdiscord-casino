    ...
    async execute(message, args, db) {
        ...
        if (user.tirages <= 0) {
            return message.reply({ 
                embeds: [createEmbed('Erreur', `Vous n'avez plus de tirages disponibles ! 🎫\n\nUtilisez \`;profil\` pour voir vos tirages restants.`, COLORS.ERROR)]
            });
        }
        ...
        try {
            ...
            if (!role) {
                return message.reply({ 
                    embeds: [createEmbed('Erreur', `Le rôle **${wonRole.name}** n'existe pas sur ce serveur.`, COLORS.ERROR)]
                });
            }
            ...
            if (member.roles.cache.has(wonRole.id)) {
                return message.reply({ 
                    embeds: [createEmbed('Déjà possédé ! 🎫', `Vous possédez déjà le rôle <@&${wonRole.id}> !\n\nVotre tirage a été consommé mais vous pouvez retenter votre chance.\n\nTirages restants: **${user.tirages - 1}** 🎫`, COLORS.ERROR)]
                });
            }
            ...
            return message.reply({ 
                embeds: [createEmbed('Erreur', `Impossible d'attribuer le rôle. Vérifiez les permissions du bot.`, COLORS.ERROR)]
            });
        }
    }
};
