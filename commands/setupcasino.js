...
module.exports = {
    ...
    async execute(message, args, db) {
        ...
        try {
            ...
            const casinoChannel = await message.guild.channels.fetch(CASINO_CHANNEL_ID);
            if (!casinoChannel) {
                return message.reply({ 
                    embeds: [createEmbed('Erreur', `Le salon CASINO n'a pas été trouvé.`, COLORS.ERROR)]
                });
            }

            // Check if casino-chat channel already exists
            let casinoChatChannel = message.guild.channels.cache.find(
                ch => ch.name === '│🎰・casino-chat' && ch.parentId === CASINO_CATEGORY_ID
            );
            ...
            let casinoRole = message.guild.roles.cache.find(r => r.name === 'Casino');
            if (!casinoRole) {
            ...
            const tiragesEmbed = createEmbed(
                ...
                `**Rôle Booster : +1 tirage/semaine**\n` +
                `**Rôle Premium : +2 tirages/semaine**\n\n` +
            ...
            message.reply({ 
                embeds: [createEmbed('Erreur', `Une erreur est survenue lors du setup : ${error.message}`, COLORS.ERROR)]
            });
        }
    }
};
